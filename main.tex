...
\begin{figure}[htbp]
    ...
    \caption{\textbf{Erasure within the minimal Heisenberg cell} (schematic). Within the symplectic quantum of action $\delta_q < q_0 : A_0 = \frac{\pi}{4} q_0 p_0 = \frac{h}{2}$, all surplus bits are erased from the thermodynamically inaccessible microstate $q_?$ specified by a program (e.g., $s_? = 110101010\dots$), collapsing to the state $\mathrm{Q}$ with the minimal program (e.g., $\mathbf{s}^* = 1101$) that runs on a universal computing machine $\mathcal{U}_{\operatorname{L}}$. This Landauer reduction constitutes a work cost of $W \ge k_B T \ln(2) |\mathbf{s}^*|$, where $|\mathbf{s}^*| = K(q)$ is the Kolmogorov complexity of the physical state.}
