...
\end{equation}
\end{subequations} 
The operationally indistinguishable microstates \(\{\mu _{q},\mu _{p}\}\) are irreversibly erased to macrostates \((q,p)\) through a deterministic mapping dictated by the information-action limit. The erasure distances, \begin{subequations}
\begin{equation}
\label{eq:epsilonq}
\epsilon_q = \mu_q - q
\end{equation} 
...
\end{equation}
\end{subequations}
preclude the classical continuum; the density of these residues provides the geometric origin of the \(|\psi |^{2}\) distribution, appearing as a 'Devil’s staircase' of quantization when plotted against the microstate coordinates.
