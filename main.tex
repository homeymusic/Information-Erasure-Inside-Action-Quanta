...
\begin{abstract}
Quantum microstates are operationally indistinguishable within finite-support Heisenberg cells. By coupling conjugate pairs of these de Gosson action quanta, we show that Landauer information erasure constrained by the least action principle prohibits the classical continuum, forcing discrete configurations and the emergence of quantum mechanics.
\end{abstract}
...
\begin{figure}[t!]
    ...
    \caption{\textbf{Information erasure inside conjugate action quanta} (schematic) showing $A_q = \frac{\pi}{4} \delta_q \Delta p \le \frac{\pi \hbar}{2}$ and $A_p = \frac{\pi}{4} \Delta q \delta_p \le \frac{\pi \hbar}{2}$ where $\delta_q=2 \hbar / \Delta p$ and $\delta_p=2 \hbar / \Delta q$ are the symplectic squeezing distances, $\Delta q$ and $\Delta p$ are from the overall action, and $\epsilon_q$, $\epsilon_p$ are the erasure distances from the microstate $\mu_{q,p}$ with binary encoding $s_q, s_p$ to the macrostate $q,p$ with minimal binary encoding $s^*_q, s^*_p$, where the bit count of the minimal binary sequences of the macrostate are less than the binary sequences of the microstate $|s^*| \le |s_\mu|$ and $\epsilon_q\le\delta_q\le\Delta q$, $\epsilon_p\le\delta_p\le\Delta p$ }
\label{fig:erase}
\end{figure}
