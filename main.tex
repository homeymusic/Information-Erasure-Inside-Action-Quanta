...
We leverage modified sequences \cite{Aiylam2013} to extend the standard Stern-Brocot tree \cite{ConcreteMath} across the entire real line, $\mathbb{R}$, by initializing the search bounds at $\pm \infty$. We simulate the physical encoding function $\mathcal{E}_U$:
...
The minimal program $s_x^* = \operatorname{reverse}(\operatorname{invert}({s_x}))$ is irreversibly destroyed, clearing the tape on $U$:
