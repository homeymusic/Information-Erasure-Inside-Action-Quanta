...
\begin{figure}[htbp]
    ...
    \caption{\textbf{Erasure within the quantum of action} (schematic). Within the symplectic quantum of action $\delta_x < x_0 : S_0 = h/2$, all surplus bits are erased from the inaccessible microstate $x_?$ having a program (e.g., $s_? = 110101010\dots$), leaving the state $x$ with the minimal program (e.g., $\mathbf{s^*} = 1101$) that runs on the Stern-Brocot universal computer $\mathcal{U}_{\operatorname{SB}}$. This reduction constitutes a Landauer erasure with a work cost of $W \ge k_B T \ln(2) |\mathbf{s^*}|$, where $|\mathbf{s^*}| = K(x)$ is the Kolmogorov complexity of the physical state.}
