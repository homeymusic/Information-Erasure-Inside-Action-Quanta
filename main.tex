...
Microstate (Initial): $x_? \in \Gamma$ \\
Macrostate (Final): $\mathrm{X} \in \mathbb{Q}$ \\
Erasure Map: $\mathcal{E}$, \quad $\Omega(\mathrm{X}) = |\{x_?\} \overset{\mathcal{E}}{\mapsto} \mathrm{X}|$\\
Hamiltonian: $\mathcal{H}$ \\

\\
\noindent
\textbf{Informational Quantities} \\
...
Classical Action: $\mathcal{A}$ \\
Quantum of Action: $\mathcal{A}_0$ \\
Relative Position: $\mathcal{L}$ \\
Relative Momentum: $\mathcal{P}$
