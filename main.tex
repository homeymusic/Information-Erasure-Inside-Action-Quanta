...
The Landauer action is defined by the algorithmic information capacity $C(\mathbf{Q}^*, \mathbf{P}^*)$, isomorphically mapped to the total bit-length $\ell$ of the identifying binary strings:

\begin{equation}
A_L = k_B T \ln(2) C(\mathbf{Q}^*, \mathbf{P}^*) \tau_\perp
\end{equation}

\begin{equation}
C(\mathbf{Q}^*, \mathbf{P}^*) \cong \ell(\mathbf{s}^*_Q, \mathbf{s}^*_P) = |\mathbf{s}^*_Q| + |\mathbf{s}^*_P|
\end{equation}
...
\begin{equation}
    A_{\mathrm{erase}} = \frac{\pi}{4} q_0 p_0 |q_?-\mathbf{Q}^*||p_? - \mathbf{P}^*|
\end{equation}
