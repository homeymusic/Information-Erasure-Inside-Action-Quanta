...
\begin{figure}[htbp]
    ...
    \caption{\textbf{Erasure within a conjugate-bounded quantum of action} (schematic). 
Within a symplectic quantum of action defined by $A_{d\Gamma} = \frac{\pi}{4} \Delta q \Delta p = \pi \hbar$, 
the binary sequences $\{s_?\}$ of operationally indistinguishable microstates $\{q_?\}$—bounded 
by the dimensionless conjugate units $\mathcal{P}=p/p_0$—are irreversibly erased by 
thermodynamic necessity. The microstates are mapped by the optimal encoder 
$\mathbf{s_0} = \mathrm{Enc}_0(q_?, \mathcal{P})$ to the macrostate 
$\mathbf{Q_0}\colon |q_? - \mathbf{Q_0}| = |\delta_q| \le \Delta q$, where $\mathbf{s_0}$ 
is the minimal binary string of length $K(\mathbf{Q_0}) = |\mathbf{s_0}|$ required to run 
the Landauer erasing machine $\mathcal{U}_{\mathrm{L}}$. This erasure over a characteristic 
time $\tau_L$ requires an action $A_{\mathrm{L}}$ bounded by the symplectic quantum 
of action: $k_{B}T\ln(2)K(\mathbf{Q_0})\tau_L \le A_L \le \pi \hbar$.}
\label{fig:erase}
\end{figure}

\section{Overview}

$$\mathbf{s_0} = \mathrm{Enc}(q_?, \mathcal{P}) \quad \text{where} \quad \mathrm{Enc} \colon \Gamma \times \mathbb{R} \to \{0,1\}^*$$
