...
\begin{figure}[t!]
    ...
    \caption{\textbf{Information erasure inside conjugate action quanta} (schematic) $A_q = \frac{\pi}{4} \delta_q \Delta p \le \frac{\pi \hbar}{2}$ and $A_p = \frac{\pi}{4} \Delta q \delta_p \le \frac{\pi \hbar}{2}$ where $\delta_q=2 \hbar / \Delta p$ and $\delta_p=2 \hbar / \Delta q$ are the symplectic squeezing distances, $\Delta q$ and $\Delta p$ are from the overall action, and $\epsilon_q$, $\epsilon_p$ are the erasure distances from the microstate $\mu_{q,p}$ with binary encoding $s_\mu$ to the macrostate $q,p$ with minimal binary encoding $s^*$ where $|s^*| \le |s_\mu|$ and $\epsilon_q\le\delta_q\le\Delta q$, $\epsilon_p\le\delta_p\le\Delta p$ }
\label{fig:erase}
\end{figure}
