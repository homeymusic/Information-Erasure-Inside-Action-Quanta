...
We show that the action available for information erasure inside Heisenberg cells is limited by
\begin{equation}
\label{eq:InfoActionLimit}
b E \tau \le \pi \hbar ,
\end{equation}
where $b = |\mathbf{s}^*|$ is the length of the minimal binary encoding $\mathbf{s}^*$ of the physical state ($q,p$), $E = k_B T \ln 2$ is the Landauer erasure energy, and $\tau$ is the characteristic time required to flip a single binary value to its orthogonal state. Here $\pi \hbar = \frac{\pi}{4} \Delta q \Delta p$ is the de Gosson quantum of action, the minimal elliptical phase-space area of finite support; unlike statistical variance $\sigma_q \sigma_p$, these dimensions $\Delta q$ and $\Delta p$ define the hard Heisenberg boundaries where states are operationally indistinguishable \cite{DeGosson2003}.
