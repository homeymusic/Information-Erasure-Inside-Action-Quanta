...
\caption{\textbf{The quantum of action} (schematic, 1D case). By geometrically translating the statistical Heisenberg limit, $\sigma_q \sigma_p \ge \hbar/2$, the classical semi-axes $\Delta q = \sqrt{2}\sigma_q$ and $\Delta p = \sqrt{2}\sigma_p$ establish the relation $\pi \Delta q \Delta p \ge h/2$. In phase space, this is the classical action area, $A \ge h/2$. Confined by these classical boundaries, the interior resolves into a conjugate pair of squeezed quantum blobs, $a_q = h/2$ and $a_p = h/2$, whose sum is the quantum of action, $h$. Specifically, $a_q$ is squeezed by the reciprocal boundary $\delta q = \hbar / \Delta p$ to area $\pi \delta q \Delta p = h/2$, while $a_p$ is squeezed by $\delta p = \hbar / \Delta q$ to area $\pi \Delta q \delta p = h/2$. These orthogonal blobs enclose their interior ellipse with quantum action area, $a \le h/2$, evaluated as $\pi \delta q \delta p = \pi \hbar^2/(\Delta q \Delta p)$. This inverse scaling geometrically enforces the correspondence principle: as the classical action area $A$ increases, the squeezed blobs $a_q=h/2$ and $a_p=h/2$ compress  into fine cross hairs, shrinking the enclosed quantum action area $a$ (see Fig.~\ref{fig:encoding_grid}). This geometric hierarchy establishes the quantum and classical relationships $a \le h/2 \le A$ and $a = (\pi \hbar)^2 / A$.}
...
$h=a_q+a_p=a_h$ with areas $a_q=\pi \delta q \Delta p = h/2$ and $a_p\pi \Delta q \delta p = h/2$.
...
Where the erasure paths from the least-action microstate $(q^\natural, p^\natural)$ to the blob centers $(q_b, p_b)$ are bounded by $|\epsilon_q^\natural| \le \delta q$ and $|\epsilon_p^\natural| \le \delta p$.
...
(x^\natural, s_x^\natural) = \operatorname{ENCODE}(x_b, \delta x)
\end{equation}

Following Bennett's protocol \cite{Bennett1973}, we invert and reverse the encoded binary path $s_x^\natural$ to form the uncomputation path $(\bar{s}_x^\natural)^R$, where both paths have bit count $N_x = |s_x^\natural| = |(\bar{s}_x^\natural)^R|$.
...
\epsilon_x^\natural = \operatorname{ERASE}(s_x^\natural, x^\natural, x_b), \quad \text{expends} \quad \Delta A_L = N_x E_L \tau
\end{equation}

As each selected microstate $(q^\natural, p^\natural)$ uncomputes to its respective center $(q_b, p_b)$, it generates the conjugate erasure displacements $\epsilon_q^\natural$ and $\epsilon_p^\natural$. The observed classical coordinates $(q,p)$ are these displacements scaled by the geometric capacity $\frac{A}{\pi \hbar}$:

\begin{equation}
\label{eq:ObservedCoordinates}
(q,p) = \frac{A}{\pi \hbar} (\epsilon_q^\natural,\epsilon_p^\natural)
\end{equation}

\section{Time Evolution and Decoherence}

The physical erasure of the selected microstate unfolds over time. As the system clears these coordinate bits, the microstate steps closer to the classical target, physically shrinking the offsets $\epsilon_q^\natural$ and $\epsilon_p^\natural$ toward zero. Because the classical coordinates scale directly with these squeezed offsets, they track this stepwise erasure process:

\begin{equation}
\label{eq:TimeEvolutionCoordinates}
q(t) = \frac{A}{\pi \hbar} \epsilon_q^\natural(t) \quad \text{and} \quad p(t) = \frac{A}{\pi \hbar} \epsilon_p^\natural(t)
\end{equation}

Quantum behavior is visible only while these bits remain. When the bits are fully erased ($\epsilon_q^\natural = \epsilon_p^\natural = 0$), the coordinates rest at the classical target $(q_b, p_b)$. We observe this mechanical loss of information as quantum decoherence.
...
\Function{ENCODE}{$\tilde{x}_b, \widetilde{\delta x}_{\text{rel}}$}
    ...
    \State $s_x^\natural \gets \text{``''}$
    \While{$|\tilde{x}^\natural - \tilde{x}_b| > \widetilde{\delta x}_{\text{rel}}$}
        \If{$\tilde{x}^\natural < \tilde{x}_b$} 
...
The engine enforces a mathematical condition for compact support; the iterative \texttt{while} loop operates under a closed-set limit, halting the algorithm the moment the relative erasure displacement $|\tilde{\epsilon}_x^\natural|$ satisfies the mapped squeezed boundary $\widetilde{\delta x}_{\text{rel}}$. This prevents artificial inflation of the Landauer action and ensures the phase-space volume is bounded by the classical action capacity. 

Once encoded, the universe must physically erase the state. For example, let $s_x^\natural = \mathtt{1101}$. Taking the bitwise complement gives $\bar{s}_x^\natural = \mathtt{0010}$, and reversing it yields the uncomputation path $(\bar{s}_x^\natural)^R = \mathtt{0100}$. By following $(\bar{s}_x^\natural)^R$, the encoded sequence $s_x^\natural$ is irreversibly destroyed. This erasure dissipates the Landauer action and yields the observable displacement:

\begin{algorithmic}[1]
\Function{ERASE}{$(\bar{s}_x^\natural)^R, \tilde{x}^\natural, \tilde{x}_b$}
    \State $N_x \gets |(\bar{s}_x^\natural)^R|$
    \While{$N_x > 0$}
        \State \Call{ShiftLeft}{$(\bar{s}_x^\natural)^R$} 
        \Statex \Comment{Expends action $k_B T \ln{2} \tau$}
        \State $N_x \gets |(\bar{s}_x^\natural)^R|$
    \EndWhile
    \State $\tilde{\epsilon}_x^\natural \gets \tilde{x}^\natural - \tilde{x}_b$ \Comment{The normalized phase-space displacement}
