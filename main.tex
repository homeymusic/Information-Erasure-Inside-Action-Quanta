...
\begin{figure}[htbp]
    ...
    \caption{\textbf{Erasure within the quantum of action} (schematic). Within the symplectic quantum of action defined by $\delta_q < \Delta q$ such that $\frac{\pi}{4} \Delta q \Delta p = \pi \hbar$, the algorithmic information of the operationally indistinguishable microstates $\{q_?\}$—specified by binary programs $\{s_?\}$ (e.g., \texttt{110101010\dots})—is irreversibly erased by thermodynamic necessity. The indistinguishable microstates collapse to the macrostate $\mathrm{Q}$ defined by the minimal binary program $\mathbf{s}^*$ (e.g., \texttt{1101}) of length $K(\mathrm{Q}) = |\mathbf{s}^*|$, running on a universal computing machine $\mathcal{U}_{\mathrm{L}}$. This Landauer erasure over a characteristic time $\tau_0$ requires an action $A_{\mathrm{L}}$ bounded by the symplectic quantum of action: $\pi \hbar \ge A_{\mathrm{L}} \ge k_{B}T\ln(2)K(\mathrm{Q})\tau_0$.}
    \label{fig:erase}
\end{figure}
